...
\startANSWER
如果所有權重均爲 \m{1/n}，則小於中位數的元素權重和爲 \m{\lfloor \frac{n-1}{2}\rfloor \frac{1}{n}}，
大於中位數的元素權重和爲 \m{\lceil \frac{n-1}{2}\rceil \frac{1}{n}}。
滿足帶權中位數的要求。
\stopANSWER
...
\startitem
如何利用線性時間的中位數算法（如\insection[worstcase_linear_selection] 中
...
\stopmathcases
\stopformula
（\hint 相互獨立的\m{\lfloor n/2\rfloor} 對元素比較開始，
然後在規模更小的子集上進行遞迴）
\stopitem
