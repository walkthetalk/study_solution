% 首行缩进
\setupindenting[first,always,0em]
% 段落间距
\setupwhitespace[medium] %none small medium big line fixed fix dimension
% 行间距
...
\defineframedtext[notepar][
  ...
  indenting={first,always,0em},	%never none not no yes always first next small medium big normal odd even DIMENSION
...
\def\startEnglish{\dostartChinese}

\long\def\dostartChinese#1\stopChinese{%
{\setscript[hanzi]
\mainlanguage[cn]
...
\def\startChineseWithoutWhiteSpace{\dostartChineseWithoutWhiteSpace}

\long\def\dostartChineseWithoutWhiteSpace#1\stopChineseWithoutWhiteSpace{%
{\setscript[hanzi]
\mainlanguage[cn]
\setupindenting[first,always,2em]%
