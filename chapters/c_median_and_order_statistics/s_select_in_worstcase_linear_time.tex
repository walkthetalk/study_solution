...
T(n) = \startmathcases
 ...
 \NC T(\left\lceil n/7 \right\rceil) + T(5n/7 + 8) + O(n) \MC \text{若 } n \ge n_0 \NR
\stopmathcases
\stopformula
...
分析 \ALGO{SELECT}，並證明：
如果 \m{n\ge 140}，則至少 \m{\left\lceil n/4 \right\rceil} 個元素大於中位數的中位數 \m{x}，
至少 \m{\left\lceil n/4 \right\rceil} 個元素小於 \m{x}。
\stopEXERCISE
