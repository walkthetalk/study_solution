...
r = /BTEX \m{\lceil n / grainsize \rceil} /ETEX
for k = 0 to r-1
...
\startigBase[a]\startitem
參考\insection[solve_system_of_linear_equation] 中的 \ALGO{LU-DECOMPOSITION}，
...
\startigBase[continue]\startitem
對\insection[solve_system_of_linear_equation] 中的 \ALGO{LUP-DECOMPOSITION} 做同樣處理。
\stopitem\stopigBase
\stopPROBLEM
...
\startigBase[continue]\startitem
對\insection[solve_system_of_linear_equation] 中的 \ALGO{LUP-SOLVE} 做同樣處理。
\stopitem\stopigBase
...
if i == j
	...
	k = /BTEX \m{\lfloor (i+j)/2 \rfloor} /ETEX
...
（與\insection[lcs] 中的 \ALGO{LCS-LENGTH} 一樣）
...
\startigBase[continue]\startitem
使用嵌套並行，多線程化\insection[rand_quicksort] 中的 \ALGO{RANDOMIZED-QUICKSORT}
...
\startigBase[continue]\startitem
分析給出的隨機快速排序的多線程算法。
（\hint 回顧\insection[selection_in_expected_linear_time] 中
