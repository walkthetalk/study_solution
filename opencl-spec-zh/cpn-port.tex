...
其行為\cnglo{impdef}，參見\refsection{reinterpret}。）
...
（請使用 \capi{upsample}，參見\refsection{integerFunc}）。
...
\item 對於數學庫函式中的大量邊界條件，比 C99 定義的更加嚴格。參見\refsection{edgeCaseBehavior}。
\stopigBase
