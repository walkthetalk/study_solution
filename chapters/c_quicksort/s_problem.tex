...
\stopformula
（\hint 以將其分成兩部分，一部分是 \m{k=2,3,\ldots,\left\lceil n/2\right\rcel-1}，
另一部分是 \m{k=\left\lceil n/2 \right\rcel,\ldots,n-1}。）
\stopitem

\startANSWER
\startformula\startmathalignment
\NC \sum_{k=2}^{n-1}k\lg{k}
   \NC=   \sum_{k=2}^{\left\lceil n/2 \right\rcel - 1}k\lg{k} + \sum_{k=\left\lceil n/2 \right\rcel}^{n - 1}k\lg{k} \NR
