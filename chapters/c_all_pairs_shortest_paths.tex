...
%p25-2
\startPROBLEM
（Shortest paths in \m{\sigma}-dense graphs）
對於圖 \m{G=(V,E)} 而言，如果 \m{|E|=\Theta(V^{1+\sigma})}，
則圖 \m{G} 爲{\EMP \m{\sigma} 稠密圖}，其中 \m{\sigma} 爲某個常數，
且 \m{0<\sigma\le 1}。
如果在 \m{\sigma} 稠密圖的最短路徑算法中使用 \m{d} 叉最小堆
（請參閱\refproblem{6-2}），
